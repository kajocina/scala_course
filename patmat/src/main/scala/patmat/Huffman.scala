...
object Huffman {
  ...
  /**
    ...
    */
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs.sortBy(_._2).map { case (c: Char, f: Int) => Leaf(c, f) }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    ...
    def diveTree(tree: CodeTree, bits: List[Bit]): List[Char] =
      ...
      tree match {
        case Fork(left, _, _, _) if bits.nonEmpty && bits.head == 0 => diveTree(left, bits.tail)
